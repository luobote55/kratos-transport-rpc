...
// generateFile generates a _mqtt.pb.go file containing gRPC service definitions.
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	g.P("// versions:")
	g.P("// - protoc-gen-go-mqtt v", version)
...
func genService(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	...
	for _, method := range service.Methods {
		...
		g.P("mqtt.REQ(r, ", fmSymbol, `, `, g.QualifiedGoIdent(method.Input.GoIdent), `{}, `, hname, `(srv))`)
	}
...
func genClientMethod(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, method *protogen.Method, index int) {
	...
	g.P(mqttPackage.Ident("ClientStream"))
	g.P("}")
	g.P()
...
func genServiceDesc(file *protogen.File, g *protogen.GeneratedFile, serviceDescVar string, serverType string, service *protogen.Service, handlerNames []string) {
	// Service descriptor.
	g.P("// ", serviceDescVar, " is the ", mqttPackage.Ident("ServiceDesc"), " for ", service.GoName, " service.")
	g.P("// It's only intended for direct use with ", mqttPackage.Ident("RegisterService"), ",")
	g.P("// and not to be introspected or modified (even as a copy)")
	g.P("var ", serviceDescVar, " = ", mqttPackage.Ident("ServiceDesc"), " {")
	g.P("ServiceName: ", strconv.Quote(string(service.Desc.FullName())), ",")
	g.P("HandlerType: (*", serverType, ")(nil),")
	g.P("Methods: []", mqttPackage.Ident("MethodDesc"), "{")
	...
	g.P("},")
	g.P("Streams: []", mqttPackage.Ident("StreamDesc"), "{")
	for i, method := range service.Methods {
		if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
			continue
		}
...
func genServerMethod(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, method *protogen.Method, hnameFuncNameFormatter func(string) string, serverType string) string {
	...
	g.P(mqttPackage.Ident("ServerStream"))
	g.P("}")
	g.P()
